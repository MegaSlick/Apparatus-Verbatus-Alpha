...
def _partition(shards: Iterable[Mapping[str, Any]]) -> tuple[dict[int, str], dict[str, list[int]]]:
    ...
    for shard in shards:
        if set(shard) != {"membership_digest", "page_ordinals"}:
            raise ContractError(
        ...
        if (
            ...
            raise ContractError("a shard boundary entry needs non-empty integer page_ordinals")
...
def boundary_records(
    ...
    *,
    ...
    for continuation in continuations:
        if set(continuation) != {"act_key", "page_ordinal", "continuation_page_ordinal"}:
            raise ContractError(
    ...
    for cluster in re_shoot_clusters:
        if set(cluster) != {"cluster_id", "member_page_ordinals"}:
        ...
        if not isinstance(pages, list) or len(pages) < 2 or pages != sorted(set(pages)):
            raise ContractError(
                "re-shoot cluster member_page_ordinals must be sorted, unique, and plural"
            )
        if any(
            not isinstance(page, int) or isinstance(page, bool) or page not in owner
            for page in pages
        ):
