...
| `imaging_ports.py` | the two vendor resize rules a witness's own preprocessing applies, as pure dimension arithmetic — carried third-party logic, cited to its pinned commit, with `common/test_vendor_parity.py` pinning both against the vendors' own functions. The Designator's structure chair and the Attestatores' Chandra and Churro adapters all size a page through it; the pixels still move through `imaging.py`'s sealed recipe |
