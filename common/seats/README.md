...
Every model the pipeline ever calls sits in a **seat**: a named role, resolved
...
|---|---|
| `models.py` | the typed values: `SeatIdentity`, `AbsentSeat`, the digest manifest, `VerifiedSnapshot`, `ServingDetails`, `ServingReceipt`, `ModelsConfig` |
...
**Nothing here substitutes.** Every refusal names the seat and the concrete
difference, and stops. There is no code path that resolves, fetches, or receipts
a seat other than the one asked for. A registry that fell back from one seat to a
close-enough one would be a picker wearing an ops hat (GOVERNANCE 3, CLAUDE.md
hard rule 8), and the closed taxonomy plus `test_seats_no_substitution.py` are
what keep one out. That test drives all seven doors through the *real* registry
and asserts, on a traced call log, that no other configured seat was reached for
...
and the artifact goes through `common/contracts/canonical.py` so a seat manifest
...
a `huggingface` seat; a `local-repository` seat never touches the network even
...
A seat configured `state = "absent"` resolves to an `AbsentSeat` — not an
exception, and not a silent omission from the roster. It stays in `run.json`'s
`witness_seats`, it earns a visible `not-run` record for every act, and it is one
...
naming the seat, never as a second route under the same role name.
