...
- [ ] After the run, bring the tree back with `verbatus fetch-run --run-id <id> --into
  ...
  volume is released; nothing under those names is derivable by the verb. Each key is the
  volume path with the `<volume>/` mount prefix removed: a key still carrying the mount
  prefix starts with `/` and is refused by name. Record the
