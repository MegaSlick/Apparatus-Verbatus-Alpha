...
def test_confirmation_refuses_an_incomplete_selector_manifest_even_for_a_retained_pair():
    ...
    # Keep the record-content digest coherent with the retained records so the
    # refusal under test (a silently dropped pair) is the one reached.
    incomplete["evidence_records_sha256"] = digest_of(
        sorted(retained, key=lambda item: tuple(item["both_digests"]))
    )
    confirmed = {
        **confirmed,
