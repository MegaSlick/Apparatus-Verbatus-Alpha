...
def _read_text(path: Path, what: str) -> str:
    try:
        ...
        raise Refusal(f"malformed-record: {what} ({path}) is not UTF-8: {error}") from error
...
def _load_jsonl(path: Path, what: str) -> list[dict[str, Any]]:
    ...
    for number, line in enumerate(_read_text(path, what).splitlines(), start=1):
        ...
        try:
            ...
            raise Refusal(f"malformed-record: {what} line {number} is not JSON: {error}") from error
        if not isinstance(row, dict):
            raise Refusal(f"malformed-record: {what} line {number} is not an object")
        rows.append(row)
    return rows
...
def _receipt(set_root: Path) -> dict[str, Any]:
    ...
    if not receipt_path.is_file():
        raise Refusal(f"missing-set-file: {receipt_path} is not a file")
    receipt_body = receipt_path.read_bytes()
    try:
        receipt = json.loads(receipt_body)
    except ValueError as error:
        raise Refusal(f"malformed-record: {receipt_path} is not JSON: {error}") from error
    if not isinstance(receipt, dict) or receipt.get("schema") != RECEIPT_SCHEMA:
        raise Refusal(f"malformed-record: {receipt_path} does not declare {RECEIPT_SCHEMA!r}")
    artifacts = receipt.get("artifacts")
    if not isinstance(artifacts, dict):
        raise Refusal(f"malformed-record: {receipt_path} names no artifacts")
    digests = {}
...
def _page_image_path(set_root: Path, image_rel: Any, page_id: str) -> Path:
    ...
    if not isinstance(image_rel, str) or not image_rel:
        raise Refusal(f"malformed-record: page {page_id!r} names no image")
...
def _positive_int(value: Any, what: str) -> int:
    if not isinstance(value, int) or isinstance(value, bool) or value <= 0:
        raise Refusal(f"malformed-record: {what} must be a positive integer, got {value!r}")
    return value
...
def _box(value: Any, what: str) -> dict[str, int]:
    if (
        ...
        raise Refusal(f"malformed-record: {what} must be four integers, got {value!r}")
    x, y, w, h = value
...
def _snapshot_index(row_snapshot: Any) -> tuple[dict[str, dict[str, Any]], str] | None:
    ...
    try:
        ...
        raise Refusal(f"snapshot-mismatch: the row snapshot does not validate: {error}") from error
...
def _cross_check_snapshot(row: dict[str, Any], sealed: dict[str, dict[str, Any]] | None) -> None:
    ...
    if sealed_row is None:
        raise Refusal(f"snapshot-mismatch: record {record_id!r} is not in the sealed row snapshot")
...
def admit_local_set(
    ...
    *,
    ...
    if split not in SPLITS:
        raise Refusal(f"unknown-split: {split!r} is not one of {sorted(SPLITS)}")
    ...
    for row in manifest_rows:
        ...
        if not isinstance(page_id, str) or not page_id:
            raise Refusal("malformed-record: a page manifest row has no page_id")
        if page_id in pages:
            raise Refusal(f"inconsistent-page-manifest: page {page_id!r} is listed twice")
        pages[page_id] = row
    ...
    for row in gold_rows:
        ...
        if set(row) != _GOLD_ROW_FIELDS:
            refuse(
                row,
                Refusal(f"malformed-record: record {record_id!r} is not the closed gold row"),
                facts=None,
            )
            continue
        if not isinstance(record_id, str) or not record_id:
            refuse(
                row,
                Refusal("malformed-record: a gold row has no record_id"),
                facts=None,
            )
            continue
        if record_id in seen_record_ids:
            refuse(
                row,
                Refusal(f"duplicate-record-id: {record_id!r} appears twice"),
                facts=None,
            )
            continue
        ...
        if page_id not in page_facts and page_id not in page_refusals:
            try:
                ...
                if not image_path.is_file():
                    raise Refusal(f"missing-page-file: {image_path} is not a file")
        ...
        try:
            ...
            if not isinstance(text, str) or not text:
                raise Refusal(f"malformed-record: record {record_id!r} carries no text")
    ...
    for page_id in sorted(by_page):
        ...
        try:
            ...
            for entry in entries:
                refuse(
                    entry["row"],
                    Refusal(f"unmintable-page-identity: {error}"),
                    facts=facts,
                )
            continue
        except CorpusRefusal as error:
            for entry in entries:
                refuse(
                    entry["row"],
                    Refusal(f"reference-build-refused: {error}"),
                    facts=facts,
                )
            continue
...
def _counts(value: Any, label: str) -> dict[str, int]:
    ...
    if not isinstance(value, dict):
        raise Refusal(f"malformed-record: {label} is not a mapping of counts")
    for name, count in value.items():
        if (
            ...
            raise Refusal(f"malformed-record: {label}[{name!r}] is not a count")
    return value
...
def validate_local_admission_ledger(ledger: Any) -> dict[str, Any]:
    ...
    if ledger["schema"] != SCHEMA:
        raise Refusal(f"wrong-schema: expected {SCHEMA!r}, got {ledger['schema']!r}")
    if ledger["corpus_id"] != CORPUS_ID:
        raise Refusal(f"wrong-corpus: expected {CORPUS_ID!r}, got {ledger['corpus_id']!r}")
    if not verify_self_hash(ledger):
        raise Refusal("self-hash-mismatch: the ledger does not hash to its own self_hash")
    if ledger["split"] not in SPLITS:
        raise Refusal(f"unknown-split: {ledger['split']!r} is not one of {sorted(SPLITS)}")
    if not isinstance(ledger["set_root"], str) or not ledger["set_root"]:
        raise Refusal("malformed-record: set_root must be a non-empty string")

    receipt = _closed(ledger["receipt"], _RECEIPT_FIELDS, "receipt")
    if not is_sha256(receipt["receipt_sha256"]):
        raise Refusal("malformed-record: receipt.receipt_sha256 must be a sha256 digest")
    ...
    for name, digest in sorted(digests.items()):
        if not is_sha256(digest):
            raise Refusal(f"malformed-record: receipt.digests[{name!r}] is not a sha256")

    snapshot = _closed(ledger["row_snapshot"], _ROW_SNAPSHOT_FIELDS, "row_snapshot")
    if not isinstance(snapshot["consulted"], bool):
        raise Refusal("malformed-record: row_snapshot.consulted must be a boolean")
    ...
    if not isinstance(snapshot["records_cross_checked"], int) or isinstance(
        ...
        raise Refusal("malformed-record: row_snapshot.records_cross_checked must be an int")
    if not snapshot["consulted"] and snapshot["records_cross_checked"] != 0:
        raise Refusal("malformed-record: no snapshot was consulted, so no record was cross-checked")

    summary = _closed(ledger["summary"], _SUMMARY_FIELDS, "summary")
    rows = ledger["rows"]
    if not isinstance(rows, list):
        raise Refusal("malformed-record: rows must be a list")
    admitted = 0
    refused = 0
    for row in rows:
...
def read_row_snapshot(path: str | Path) -> dict[str, Any]:
    ...
    path = Path(path)
    if not path.is_file():
        raise Refusal(f"missing-set-file: {path} is not a file")
    try:
