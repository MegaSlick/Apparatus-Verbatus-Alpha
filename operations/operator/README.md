...
them, `boot`, `run`, `export`, and `close` the moment you are done. Run `status` any time
you are unsure what is happening or costing money.
...
Everything this tool writes for itself lives in one folder, `.verbatus/` in the project
folder by default; `--state-dir` moves it. Each record is written once and named after a
