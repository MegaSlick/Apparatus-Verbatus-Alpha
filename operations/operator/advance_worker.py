...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        return 2
    # The record above is on disk and permanent; a closed or broken stdout
    # pipe here must not exit as an uncaught traceback the parent would read
    # as "the advance was refused" about a boundary that was in fact advanced.
    try:
        sys.stdout.write(json.dumps(reference.to_record(), sort_keys=True) + "\n")
        sys.stdout.flush()
    except OSError as error:
        # The interpreter's own flush at shutdown would otherwise raise again
        # on the unsent report and replace this status with 120; pointing the
        # descriptor at the null device discards that second attempt only.
        try:
