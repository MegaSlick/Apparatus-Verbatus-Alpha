...
def _frame(name: str) -> SubmittedFrame:
    image = Image.new("L", (64, 48), 90 + ord(name[0]))
    encoded = BytesIO()
...
class _Batch:
    ...
    def __init__(self, tmp_path: Path, names: tuple[str, ...] = ("a", "b")):
        ...
        # This fixture exercises the console's candidate path; the instrument schema
        # intentionally records a verdict without turning it into a page-identity fact.
        for record in self.evidence:
            record["verdict"] = "complementary-candidate"
        self.paths = {
