...
def _account_for_declared_act(
    ...
    row = {
        "act_id": act_id,
        "act_key": act["key"],
        # The sealed page record's own subject where the page sealed -- the
        # same string `page_identity` derives for a fixture page (both are
        # `page_id` over the source digest; `test_structure_pass.py` pins the
        # equality for every sealed fixture page), but only one of them exists
        # for a real page. A page that never sealed has no sealed subject, and
        # this loop is the fixture path alone, so the fixture's declared
        # identity is the one name such a page still has here.
        "page_id": (
