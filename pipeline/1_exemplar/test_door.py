...
def _admitted_re_shoot_pair(tmp_path, register_bytes=None):
    ...
    tree, context = open_door(tmp_path, sources, register_bytes=register_bytes)
    assert (
        process_sources(
            context,
            tree,
...
def test_the_loud_failure_names_the_reasons_rather_than_counting_anonymously(tmp_path):
    ...
    assert "all 2 submitted source(s) were refused" in message
...
def test_cluster_report_keeps_refused_members_and_parts_visible(tmp_path):
    ...
    payload = json.loads(tree.read_bytes(report_path.path))["payload"]
