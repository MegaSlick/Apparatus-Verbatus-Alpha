...
def test_a_fabricated_render_transform_is_refused_rather_than_sealed_or_crashed(tmp_path):
    ...
    carrying a fabricated `container_page_index` was never checked, and the payload
    builder then read `container_sha256` beside it and died with a bare KeyError. A
    record claiming a transform that did not happen is the same untruth as a
    duplicate reason claiming an admission that did not."""
    tree, _ = build_door_run(tmp_path / "runs")
    identity = artifact_id(DOOR, "admission", "source-1")
    path = tree.resolve(tree.artifact_path(DOOR, "admission", identity))
    record = json.loads(path.read_text(encoding="utf-8"))
    record["payload"]["container_page_index"] = 0
    path.write_bytes(canonical_bytes(record))
    tree.write_manifest(DOOR)
