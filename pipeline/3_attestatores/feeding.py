...
def retain_model_view(
    ...
    *,
    ...
    if adapter == "churro.v1":
        # The XML closing tag makes adjacent raw tail windows unequal, so parsed
        # text is inspected when available. The raw blob is already immutable.
        if parser == "xml":
            try:
                record["parse"] = {
                    "state": "parsed",
        ...
        if finding := detect_repetition(inspected):
            ...
            # Parser refusal remains the primary stop reason; repetition is
            # retained independently in findings.
