...
def _read_the_acts(registry_factory, serving_factory, service: ResidentChair) -> int:
    ...
    # loop below starts a chair the first time an act actually needs a reading,
    # so a resumed pass whose acts are all already sealed never loads a 27B
    # model onto a card that bills by the hour to read nothing. `service` owns
    # the shutdown from the moment the client exists.
    receipt_ref: dict[str, str] | None = None
