...
def _shrinking_reader(context, target: str):
    ...
    def read(stage, kind, artifact_id):
        ...
        if stage != DESIGNATOR or kind != "region" or artifact_id != target:
            return record
...
def _witness_moving_reader(inner):
    ...
    def read(stage, kind, artifact_id):
        ...
        disagreement = payload.get("partition_disagreement") or {}
        for observation in disagreement.get("unclaimed_observations") or []:
            observation["bounds"]["x"] += WITNESS_SHIFT_X
