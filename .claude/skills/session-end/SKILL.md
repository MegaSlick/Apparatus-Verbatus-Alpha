...
4. **Park.** If the work continues, stay on its branch. If its pull request has merged,
   move to a fresh branch from `origin/main`; delete the old branch only if the pull
   request's head commit equals the branch tip. Remove an agent's worktree only once its
   work is merged or recorded.
5. **Report and notify.** Give the final state, checks, external actions and the next
   step, then send the `done` notification with `operations/notify/notify.sh`. It prints
   nothing on success.
